...
在执行任何代码编写之前，AI 必须检查以下文件是否存在且内容完整：
- `docs/product-design.md`（Phase 1）
- `docs/tech-architecture.md`（Phase 2）
- `docs/visual-standard.md`（Phase 3）

**如果缺失，拒绝写代码**，转而引导用户通过 `/new-project` skill 完成前置阶段。

新项目启动时，自动触发 `new-project` skill 进行阶段检查和引导。
...
每次用户表示功能完成（"done", "finished", "完成了"等）或准备提交代码时，AI 必须：
1. 自动分析哪些文档受影响
2. 生成具体的更新检查清单
3. 询问是否立即更新
4. 如用户同意，立即执行文档更新

优先使用 `post-feature` skill 自动化此流程。
